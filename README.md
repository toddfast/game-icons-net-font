...
This change required using a patched version of FontCustom to adjust the default starting code point in order to ensure all icons could fit within the Unicode Private Use Area. The required patch is in the `fontcustom-2.0.0` Ruby gem, on line 47 of `fontcustom-2.0.0/lib/fontcustom/generator/font.rb` (on Mac, you can find this file in `/Library/Ruby/Gems/<version>/gems/`). Without this change, the resulting font file would not work on Windows and could experience other compatibility issues.